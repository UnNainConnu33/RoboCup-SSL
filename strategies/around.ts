      ...
      for (let i = 0; i < this.robots_amount; i++){
        ...
        if (this.opponent_id == -1) {     // if opponent_id == -1 the robots go around the ball
          void broker.call('bots-control.moveTo', {
            ...
            // target: {x: shifts[i][0] + state.data.ball.position.x, y: shifts[i][1] + state.data.ball.position.y},
            orientation: 0,
            expectedReachTime: 10,
        } as MoveToPacket)
    }
        else {
          void broker.call('bots-control.moveTo', {   // here, the robots go around the opponent's robot specified
            id: i,
            target: {x: cos(2 * pi / this.robots_amount * i) * this.radius + state.data.robots.opponents[this.opponent_id].position.x, 
                     y: sin(2 * pi / this.robots_amount * i) * this.radius + state.data.robots.opponents[this.opponent_id].position.y },
            orientation: 0,
            expectedReachTime: 10,
        } as MoveToPacket)
    }
      ...
      for (let i = 0; i < this.robots_amount ; i++){
        switch (this.robots_amount) {
          ...
          case 3:
            void broker.call('bots-control.moveTo', {
              id: i,
              target: {x: shifts_3[i][0] + state.data.ball.position.x, y: shifts_3[i][1] + state.data.ball.position.y},
              orientation: 0,
              expectedReachTime: 10,
          } as MoveToPacket)
              break
          ...
          case 5:
            void broker.call('bots-control.moveTo', {
              id: i,
              target: {x: shifts_5[i][0] + state.data.ball.position.x, y: shifts_5[i][1] + state.data.ball.position.y},
              orientation: 0,
              expectedReachTime: 10,
          } as MoveToPacket)
              break
          ...
          case 6:
            void broker.call('bots-control.moveTo', {
              id: i,
              target: {x: shifts_6[i][0] + state.data.ball.position.x, y: shifts_6[i][1] + state.data.ball.position.y},
              orientation: 0,
              expectedReachTime: 10,
          } as MoveToPacket)
              break
