...
/**
 * call "MSB.MoveTo" '{ "robots_amount" : 5, "opponent_id" : 0, "radius" : 1}'
 */
  ...
  compute(broker: ServiceBroker): boolean {
      ...
      for (let i = 0; i < this.robots_amount; i++){
        void broker.call('bots-control.moveTo', {
            id: i,
            target: {x: Math.cos(2 * Math.PI / this.robots_amount * i) * this.radius + state.data.robots.opponents[this.opponent_id].position.x, 
                y: Math.sin(2 * Math.PI / this.robots_amount * i) * this.radius + state.data.robots.opponents[this.opponent_id].position.y},
            orientation: 0,
...
export default class MoveTo extends Strategies {
  ...
  compute(broker: ServiceBroker): boolean {
      ...
      state.data.robots.opponents[0].position
      ...
      for (let i = 0; i < 4; i++){
        void broker.call('bots-control.moveTo', {
            id: i,
            target: {x: shifts[i][0] + state.data.robots.opponents[0].position.x, y: shifts[i][1] + state.data.robots.opponents[0].position.y},
            orientation: 0,
            expectedReachTime: 10,
        } as MoveToPacket)
    }
